...
from tensorflow.keras import backend as KB, Model, Input, optimizers
# pylint: disable=E0611,E0401
...
class ResidualBlock(Layer):
    ...
    def build(self, input_shape):
        ...
        with KB.name_scope(self.name):  # name scope used to make sure weights get unique names
            self.layers = []
            self.res_output_shape = input_shape

            for k in range(2):  # dilated conv block.
                name = 'conv1D_{}'.format(k)
                with KB.name_scope(name):  # name scope used to make sure weights get unique names
                    conv = Conv1D(
                    ...
                    if self.use_weight_norm:
                        ...
                        with KB.name_scope('norm_{}'.format(k)):
                ...
                with KB.name_scope('act_and_dropout_{}'.format(k)):
    ...
    def call(self, inputs, training=None, **kwargs):
        ...
        for res_block in self.residual_blocks:
            try:
                ...
                x, skip_out = res_block(KB.cast(x, 'float32'), training=training)
        ...
        if not self.return_sequences:
            ...
            if self.padding_same_and_time_dim_unknown:
                self.output_slice_index = KB.shape(self.layers_outputs[-1])[1] // 2
...
def compiled_tcn(num_feat,  # type: int
    ...
    if not regression:
        ...
        def accuracy(y_true, y_pred):
            ...
            if KB.ndim(y_true) == KB.ndim(y_pred):
                y_true = KB.squeeze(y_true, -1)
            # convert dense predictions to labels
            y_pred_labels = KB.argmax(y_pred, axis=-1)
            y_pred_labels = KB.cast(y_pred_labels, KB.floatx())
            return KB.cast(KB.equal(y_true, y_pred_labels), KB.floatx())
