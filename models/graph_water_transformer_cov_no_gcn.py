...
def graph_water_transformer_cov_gate_predictor_no_gcn(input_shape, lstm_unit, num_transformer_blocks, head_size, num_heads, ff_dim, atte_reg, l1_reg, l2_reg, dropout, masked_value, gate_min, gate_max):
    """
    transformer_cov
    graph_water
    """
    ...
    cov = Dense(lstm_unit)(cov)  # (96, 5)
    cov = layers.MaxPooling1D(pool_size=2, padding='same', name='pooling1')(cov)

    
    # ======================== water levels with GNN ========================
    inp_lap = Input((5, 5), name='inp_lap')
    inp_seq = Input((5, 72), name='inp_seq')
    inp_seq_reshape = layers.Reshape((72, 5), name='inp_seq_reshape')(inp_seq)
    ...
    # RNN
    xx = LSTM(lstm_unit, activation='relu', return_sequences=True, name='LSTM')(inp_seq_reshape)  # (72, 5)
    xx = layers.MaxPooling1D(pool_size=2, padding='same', name='pooling2')(xx)
    
    
    # ======================== CONCAT and Attention ========================
    x = Concatenate(name='concate', axis=1)([cov, xx])
    ...
    model = Model([cov_inputs, inp_seq], out)
